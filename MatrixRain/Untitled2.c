...
{
    printf("couldn't initialize image or primitives addon\n");
    return 1;
}
    ...
    while(1)
    {
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            al_draw_tinted_bitmap(mysha, al_map_rgba_f(0.5, 0.2, 0.5, 0.5), 300, 300, 0);
            al_draw_filled_rectangle(240, 260, 340, 340, al_map_rgba_f(0, 0, 0.5, 0.6));

        al_draw_circle(450, 370, 80, al_map_rgb_f(1, 0, 1), 2);
            al_draw_text(font, al_map_rgb(25, 255, 255), 10, 0, 0, "Hello world!");
            ALLEGRO_VERTEX v[] = {
            { .x = 90, .y = 320, .z = 0, .color = al_map_rgb_f(1, 0, 0) },
...
int main()
{
    ...
    BOUNCER obj[BT_N];             // making a 8 bouncer object
    for(int i = 0; i < BT_N; i++)
    {

        obj[i].x =rand()%640;
        obj[i].y =rand()%480;
        obj[i].dx = ((((float)rand()) / RAND_MAX) - 0.5) * 2 * 4;
        obj[i].dy = ((((float)rand()) / RAND_MAX) - 0.5) * 2 * 4;
        obj[i].type = i;
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            for(int i = 0; i < BT_N; i++)
            {
                ...
                switch(b->type)
                {
                    ...
                    case BT_RECTANGLE_3:
                        al_draw_filled_rectangle(b->x , b->y, b->x + 200, b->y + 80, al_map_rgba_f(32, 425, 0.5, 0.5 ));
                        break;
      ...
      switch(event.type)
{
    ...
    case ALLEGRO_EVENT_MOUSE_AXES:
        dx += event.mouse.dx * 0.5;
        dy += event.mouse.dy * 0.5;
